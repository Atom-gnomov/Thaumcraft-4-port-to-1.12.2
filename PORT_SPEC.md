...
> **Исторический документ ранних фаз (по ~1.0.x).** Статусы карточек ниже
> (`TODO Phase 2/3`, `STUB`) давно недостоверны: выборочная проверка 2026-08-02
> показала, что помеченное «TODO» частью давно сделано (например, «точный TESR
> кристаллов = TODO Phase 3» — `TileCrystalRenderer` существует). Реверс-заметки
> о поведении оригинала остаются полезными; статусам верить нельзя — текущее
> состояние смотреть в `CHANGELOG.md` и `KNOWN_ISSUES.md`.
