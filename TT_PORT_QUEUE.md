...
> **Очередь пуста — переносить нечего (на 1.1.42.0).** Последний узел (врата,
> небесная жемчужина, фокус отзыва) закрыт; разбор ниже оставлен как запись
> оригинала, а не как план.
>
> Перегенерировать скриптом больше нельзя: `scripts/gen_tt_queue.py` читает
> `../tt-original-1.7.10`, которого на машине нет. Открытые расхождения модуля
> живут теперь в [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) и
> [`THAUMIC_TINKERER_PLAN.md`](THAUMIC_TINKERER_PLAN.md), а не здесь.

Сгенерировано скриптом `scripts/gen_tt_queue.py` из исходников оригинала
(`../tt-original-1.7.10`, пока он был на месте) и дерева этого порта.

**Состояние: портировано 79 из 79.**
