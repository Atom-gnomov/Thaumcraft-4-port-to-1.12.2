...
**Что в работе сейчас:** цепочка «яркий нитор → шесть элементальных костров»
(очередь `TT_PORT_QUEUE.md`, №15 и №39–44). Один предмет открывает семь
объектов, поэтому берётся первым, а не по номеру в таблице.

**Состояние модуля:** 39 из 84 объектов каталога, версия 1.1.8.0.
