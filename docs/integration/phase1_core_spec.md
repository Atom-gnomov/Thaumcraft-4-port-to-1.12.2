...
public final class EnergyCanon {
    ...
    // алюментум: кода не нужно — burn time 6400 в порте, ~16,000 EU в генераторе
...
```
- `THAUM_IC2_INTRO`: скрытое. Реализация (уточнено по коду порта — хук НЕ
  нужен): у порта есть родные item/aspect-триггеры скрытых исследований
  (`ResearchItem.setItemTriggers/setAspectTriggers` + `ResearchManager.createClue`
  на сервере при скане). Ставим item-триггеры на машины IC2 (совпадение по
  предмету с ignoreDamage — любой блок `te` подходит); широкий аспект-триггер
  Machina — только аварийный фолбэк, иначе вкладку открывал бы ванильный поршень.
  Нюанс: за скан выдаётся одна случайная зацепка из всех подходящих скрытых
  исследований, и скан требует знания родительских аспектов (validScan).
...
```
- Алюментум-топливо: порт 1.2.8.0 вернул burn time 6400 тиков (парити TC4),
  генератор IC2 принимает его сам; ожидание ~16,000 EU (замерить на стенде,
  приёмка: 16,000 ± 5%).
...
| Хук | Для чего | Статус |
|---|---|---|
| ~~Событие «предмет отсканирован»~~ | триггер THAUM_IC2_INTRO | **не нужен**: у порта есть родные триггеры (§5) |
| ~~Геттер burn-предметов алхимии~~ | алюментум-топливо | **не нужен**: порт 1.2.8.0 вернул burn time алюментума |
| ~~TileNitor implements IHeatSource~~ | нитор-тепло 20 HU/t (фаза 2) | **сделан в 1.2.8.1**: потребители тепла IC2 ищут источник жёстким `instanceof` по соседнему TE — извне интерфейс не навесить; @Optional + deobfProvided IC2 |
| (фаза 3+) API ауры чанка: get/add/drain + flux | конвертеры, скруббер | заявить заранее |
| (фаза 3+) регистрация cap/rod жезлов извне | компоненты жезлов | заявить заранее |

Итог: фаза 1 обошлась БЕЗ правок TC4U (кроме парити-фикса топлива);
фаза 2 потребовала одну (нитор-IHeatSource, 1.2.8.1) — таблица уточнена.
...
5. Алюментум в генераторе даёт ~16,000 ± 5% EU (burn time 6400 из порта).
