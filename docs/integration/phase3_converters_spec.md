...
- **Темп (пересчитано по коду, ИСПРАВЛЕНИЕ прежней ошибки в этом файле):**
  - *Всплеск*: узел отдаёт накопленное — 2,000 EU за цикл в 20 тиков =
    100 EU/t на бумаге, но эмиссия ограничена тиром LV (**32 EU/t**).
    Обычный узел (~50 Ignis, пол 10) отдаёт ~40 вис = 80,000 EU.
  - *Долговременно*: узел восполняет +1 вис за **600 тиков** (BRIGHT 400,
    PALE 900, FADING — никогда) ⇒ **3.3 EU/t с обычного узла**, 5 EU/t с
    яркого. Прежняя правка этого файла утверждала «~66 EU/t» — это была
    ошибка на 20× (перепутаны EU/сек и EU/тик).
  - Следствие для баланса: **генератор — ручеёк, а не электростанция**.
    Чтобы держать 32 EU/t постоянно, нужно ~10 обычных узлов в кубе ±8
    (или скармливать узлы Эфирному Двигателю от избытка «грязной» энергии).
    Строка канона v3 «32/128 EU/t» описывает всплеск, а не устойчивый темп.
    Лорные страницы («капля в полминуты») это уже отражают.
- **Антиферма**: интерференция по кэшу позиций (реестр генераторов, не скан
  мира): другой генератор на евклидовом расстоянии **≤16 блоков** глушит
  оба. ПКМ пустой рукой по машине выдаёт строку статуса (буфер или
  «резонансная интерференция») — минимальная замена GUI в фазе 3а.
...
```
THAUM_ORE_MACERATION → THAUM_STEEL           [тигель: сталь IC2 + Metallum/Praecantatio]
THAUM_STEEL          → VIS_TO_EU_GENERATOR   [арк. верстак: рецепт блока]
VIS_TO_EU_GENERATOR  → EU_TO_VIS_ENGINE      [арк. верстак]
VIS_TO_EU_GENERATOR  → LORE_RESONANCE_LIMITS [лорное, secondary]
```
**Ревизия при реализации:** Таум-Сталь поднята из фазы 3в в 3а и сделана
настоящим предметом (а не временной заменой таумием) — она нужна обоим
рецептам как корпус, и держать «временный таумий» с TODO дороже, чем сразу
сделать слиток. Отсюда лишний узел в дереве и родитель `THAUM_STEEL`
у генератора.

Рецепты (арканный верстак, `ThaumcraftApi.addArcaneCraftingRecipe`):
- Генератор: генератор IC2 + 4× таум-сталь + 2× ртуть (проводник ауры) +
  2× схема IC2.
...
1. Генератор у узла: копит EU, кабель уносит; узел не падает ниже 20%
   по-аспектной ёмкости (и никогда ниже 1 — защита от узлов с нулевой
   ёмкостью аспекта); после выработки «излишка» темп падает до ~3.3 EU/t.
2. Второй генератор на расстоянии ≤16 блоков — оба глушатся; ПКМ по машине
   сообщает «резонансная интерференция».
