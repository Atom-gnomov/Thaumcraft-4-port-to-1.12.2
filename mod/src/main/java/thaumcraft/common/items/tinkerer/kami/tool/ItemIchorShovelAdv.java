...
/**
 * Awakened Ichorium Shovel — ported from Thaumic Tinkerer's
 * {@code ItemIchorShovelAdv} (pixlepix/nekosune, originally Vazkii).
 *
 * <p>Three modes: <em>Block</em>, <em>Square</em> (a five-by-five slab across
 * the face struck) and <em>Column</em>, which digs a sixteen-tall shaft from
...
public class ItemIchorShovelAdv extends ItemIchorShovel implements IAdvancedTool {
    ...
    public boolean onBlockStartBreak(ItemStack stack, BlockPos pos, EntityPlayer player) {
        ...
        if (!KamiToolHandler.isRightMaterial(state.getMaterial(), KamiToolHandler.MATERIALS_SHOVEL)) {
            return false;
        }
        ...
        switch (KamiToolHandler.getMode(stack)) {
            case 1: {
                ...
                KamiToolHandler.removeBlocksInIteration(player, world, pos,
                        ...
                        null, KamiToolHandler.MATERIALS_SHOVEL, silk, fortune);
                break;
            }
            case 2:
                // Column: only blocks matching the one struck, eight down to seven up.
                KamiToolHandler.removeBlocksInIteration(player, world, pos,
                        0, -8, 0, 1, 8, 1,
                        state.getBlock(), KamiToolHandler.MATERIALS_SHOVEL, silk, fortune);
                break;
            default:
                break;
        }
